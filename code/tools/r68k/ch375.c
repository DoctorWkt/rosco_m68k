...
#define WR_USB_DATA1	0xFF	// Not a real cmd, just to tell us
				// that we already have the maxcnt
...
// The max count is what the CPU told us it would send.
...
uint8_t read_ch375_data(void) {
  ...
  if (prevcmd == GET_STATUS) return (status);
...
// Send a command to the CH375. If the function returns true,
// then the CPU must generate an interrupt.
uint8_t send_ch375_cmd(uint8_t cmd) {
  ...
  if (logfh != NULL && (loglevel & LOG_CH375) == LOG_CH375) {
    ...
    switch (cmd) {
      ...
      case RESET_ALL: fprintf(logfh, "RESET_ALL\n"); break;
      case CHECK_EXIST: fprintf(logfh, "CHECK_EXIST\n"); break;
      case SET_USB_MODE: fprintf(logfh, "SET_USB_MODE\n"); break;
      case TEST_CONNECTION: fprintf(logfh, "TEST_CONNECTION\n"); break;
      case GET_STATUS: fprintf(logfh, "GET_STATUS\n"); break;
      case RD_USB_DATA: fprintf(logfh, "RD_USB_DATA\n"); break;
      case WR_USB_DATA: fprintf(logfh, "WR_USB_DATA\n"); break;
      case WR_USB_DATA1: fprintf(logfh, "WR_USB_DATA1\n"); break;
      case DISK_INIT: fprintf(logfh, "DISK_INIT\n"); break;
      case DISK_SIZE: fprintf(logfh, "DISK_SIZE\n"); break;
      case DISK_READ: fprintf(logfh, "DISK_READ\n"); break;
      case DISK_RD_GO: fprintf(logfh, "DISK_RD_GO\n"); break;
      case DISK_WRITE: fprintf(logfh, "DISK_WRITE\n"); break;
      case DISK_WR_GO: fprintf(logfh, "DISK_WR_GO\n"); break;
      default: fprintf(logfh, "0x%x\n", cmd);
    }
  }

  switch (prevcmd = cmd) {
  case GET_IC_VER:
    bufindex = 0; bufcnt = 1; buf[0] = 0xB7; status = 0;
    break;
  case RESET_ALL:
    bufindex = 0; bufcnt = 0; status = 0;
    break;
  case CHECK_EXIST:
    bufindex = 0; bufcnt = 0; status = 0;
    break;
...
// Send data to the CH375. If the
// function returns true, then the CPU
// must generate an interrupt.
uint8_t send_ch375_data(uint8_t data) {
  off_t offset;
  int err;
  ...
  case CHECK_EXIST:
    ...
    bufindex = 0; bufcnt = 1; buf[0] = ~data;
    break;
  case SET_USB_MODE:
    // Ensure it's a 6
    if (data != 6) {
  ...
  case WR_USB_DATA1:
    // Check that we haven't exceeded the maximum data from the CPU
    if (bufindex == maxcnt) {
	...
	fprintf(stderr, "CH375 write error\n"); exit(1);
      ...
      fflush(disk); bufindex = 0; maxcnt = 0;
    }
    break;
